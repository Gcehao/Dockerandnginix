...
installApps()
{
    ...
    if [[ "$ISCOMP" == *"command not found"* ]]; then
        read -rp "是否安装 Docker-Compose（y/n）: " DCOMP
    else
    ...
    read -rp "安装NGinX 代理管理器（是/否）: " NPM
    read -rp "否安装Navidrome音乐软件: " NAVID
    read -rp "否要安装 Portainer-CE容器管理器: " PTAIN
